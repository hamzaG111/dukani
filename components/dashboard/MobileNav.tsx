        ...
        {nav.map((item) => {
          ...
          return (
            ...
            >
              ...
              <span className={`relative transition-colors ${active ? "text-gold" : "text-muted"}`}>
                {item.icon(active)}
              </span>
              <span className={`relative text-[10px] font-semibold ${active ? "text-gold" : "text-muted"}`}>
                {item.label}
              </span>
              {item.badge && !active && (
                <span className="absolute top-0.5 right-1.5 w-4 h-4 rounded-full bg-gold text-background text-[9px] font-black flex items-center justify-center">
                  {item.badge}
                </span>
              )}
            </a>
